...
C_FILE=${1:-bpf/sched_ext.bpf.c}
BASE_FILE=$(basename ${C_FILE})

# if --help is passed, print the usage

if [ "$1" = "--help" ]; then
    echo "Usage: ./start.sh bpf/scheduler_file.c"
...
sudo bpftool struct_ops register obj/${BASE_FILE}.o /sys/fs/bpf/sched_ext || (echo "Error attaching scheduler, consider calling stop.sh before" || exit 1)
